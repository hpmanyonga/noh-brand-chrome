...
1. **Network One Health leads by default.** In headers, hero copy, page titles, and anywhere visible to users, NOH is the front brand. The practice name "Dr HP Manyonga and Associates Inc." appears ONLY in the compliance footer — never in chrome or hero. **EXCEPTION (HP override 2026-07-04): the clinician-owned provider estate** leads with **"Dr H P Manyonga & Associates"** and presents NOH as "the Network One Health provider network" — set via `lead="practice"` on `BaseLayout`/`Header` (default `lead="noh"`). It reinforces clinician ownership to the specialist audience.

   **Scoped 2026-08-30.** The exception applies to **`join.hpmanyonga.co.za` only**. It originally named `hpmanyonga.co.za` too, and that half was never implemented: all 26 apex pages have always shipped `lead="noh"`, all 5 join pages `lead="practice"`. The code was right and the rule was wrong, for three reasons.

   - The exception already carved out patient-facing surfaces, and HP's 50:30:10:10 ruling made the apex front door the patient's. It is now a patient-facing surface by its own definition.
   - HP 2026-08-30: NOH is **always** presented as the envelope. That is the later and more specific instruction.
   - The apex sells NOH-named service-products (NOH Maternity Programme, SWIFT Care, ColpoCare, Well Woman 360). Network One Health is the management company those names derive from, so putting the practice's masthead over a page selling them is incoherent.

   The exception's other purpose, de-conflicting with `networkonehealth.co.za`, is now served properly and structurally: `sameAs` names the brand home first, and `llms.txt` states which site answers which question. Splitting the masthead across two patient-facing domains would confuse patients rather than de-conflict a market.
